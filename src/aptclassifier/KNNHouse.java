 ...
 */
public class KNNHouse
{
    private int k; //number of neighbors
    private ArrayList<Object> labels; //set of classifications for houses
    private ArrayList<Property> houses; //collection of houses
    static Houses house = new Houses(); //instance of House class
     ...
     */
    public KNNHouse(ArrayList<Property> houses, int k){
        ...
        this.houses = houses; //list of apartments each time 'this.' is called

        //cycle through apartments
        for(Property query : houses){
        ...
        for(Property x : this.houses){
            ...
            if(result[result.length-1] == null){ //if not filled
                ...
                if(distance < knearest){//if distance smaller then the last known smallest
                    result[index] = x;//set current property in whatever index loop is on
    ...
    public Object classify(Property query){
        ...
        //             String value = neighbors.get(o).toString();
